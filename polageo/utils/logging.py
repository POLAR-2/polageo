...
polageo_dev_log_handler = handlers.TimedRotatingFileHandler(
...
polageo_dev_log_handler.setFormatter(_dev_formatter)
polageo_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

polageo_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

polageo_usr_log_handler.setLevel(logging.INFO)
...
polageo_usr_log_handler.setFormatter(_usr_formatter)
...
polageo_console_log_handler = logging.StreamHandler(sys.stdout)
polageo_console_log_handler.setFormatter(_console_formatter)
polageo_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    polageo_usr_log_handler.addFilter(warning_filter)
    polageo_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    polageo_usr_log_handler.removeFilter(warning_filter)
    polageo_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    polageo_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(polageo_dev_log_handler)

    log.addHandler(polageo_console_log_handler)

    log.addHandler(polageo_usr_log_handler)
